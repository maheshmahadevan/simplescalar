...
struct cache_blk_t *blk) /* block to unlink */
{
	...
	for (prev = NULL,ent = set->hash[index]; ent;
...
cache_create(
		...
		unsigned int(*blk_access_fn)(enum mem_cmd cmd, md_addr_t baddr,
				int bsize, struct cache_blk_t *blk, tick_t now,
				md_addr_t* prev_addr), unsigned int hit_latency) /* latency in cycles for a hit */
{
	struct cache_t *cp;
	...
	cp = (struct cache_t *) calloc(1,
			sizeof(struct cache_t) + (nsets - 1) * sizeof(struct cache_set_t));
	if (!cp)
...
FILE *stream) /* output stream */
{
	...
	fprintf(
			...
			cp->name,
			cp->assoc,
			cp->policy == LRU ? "LRU" : cp->policy == Random ? "Random" :
			cp->policy == FIFO ? "FIFO" : (abort(), ""));
}
...
struct stat_sdb_t *sdb) /* stats database */
{
	...
	stat_reg_counter(sdb, buf, "total number of writebacks", &cp->writebacks, 0,
			NULL);
	...
	stat_reg_formula(sdb, buf, "replacement rate (i.e., repls/ref)", buf1,
			NULL);
	...
	stat_reg_formula(sdb, buf, "invalidation rate (i.e., invs/ref)", buf1,
			NULL);
}
...
FILE *stream) /* output stream */
{
	...
	fprintf(stream,
			...
			cp->name, (double) cp->misses / sum,
			(double) (double) cp->replacements / sum,
...
struct cache_blk_t *prev_blk) /*maheshma - SVC - for previous block victim in l1 cache*/
{
	...
	//maheshma - SVC changes - commented the below code as it is not used anywhere
	// and our code will use this as the address sent from upper level cache
	/*if (repl_addr)
	 *repl_addr = 0;*/
	...
	if (strcmp(cp->name, cache_dl1->name) == 0
			|| (cache_dl2 && strcmp(cp->name, cache_dl2->name) == 0)
			|| (cache_dvc && strcmp(cp->name, cache_dvc->name) == 0)) {
		cache_vc = cache_dvc;
		cache_l1 = cache_dl1;
	} else if (strcmp(cp->name, cache_il1->name) == 0
			|| (cache_il2 && strcmp(cp->name, cache_il2->name) == 0)
			|| (cache_ivc && strcmp(cp->name, cache_ivc->name) == 0)) {
		cache_vc = cache_ivc;
		cache_l1 = cache_il1;
	}
	...
	if ((cache_dvc && strcmp(cp->name, cache_dvc->name) == 0)
			|| (cache_ivc && strcmp(cp->name, cache_ivc->name) == 0)) {
		cp->hsize = 0;
	}
	...
	if (strcmp(cp->name, cache_dl1->name) == 0
			...
			|| strcmp(cp->name, cache_il1->name) == 0
			|| strcmp(cp->name, cache_il2->name) == 0) {
	...
	if (selective_vc && cmd == Read
		...
		if (prev_blk->sticky_bit == 0) {
			...
			//Main cache blk should have valid status as this is new block.
			prev_blk->status = CACHE_BLK_VALID;
	...
	if (((cache_dvc && strcmp(cp->name, cache_dl1->name) == 0)
			|| (cache_ivc && strcmp(cp->name, cache_il1->name) == 0))
			&& repl->updated == 0) {
		...
		//LRU		
		repl_vc = cache_vc->sets[set_vc].way_tail;
	...
	//maheshma-11/30 - Implementing swap during Simple VC hit sceanrio.
	//if it is hit in VC , victim from l1 is swapped with VC
	if (cmd == Read
			&& ((cache_dvc && strcmp(cp->name, cache_dvc->name) == 0)
					|| (cache_ivc && strcmp(cp->name, cache_ivc->name) == 0))) {
		move(*repl_addr, blk, cache_vc);
		move(addr, prev_blk, cache_l1);
		//maheshma - also need to swap the block status so if there is any Dirty block set it is appropriately moved/handled
		unsigned int temp_status = blk->status;
		blk->status = prev_blk->status;
		prev_blk->status = temp_status;
		prev_blk->updated = 1;
	}
	...
	if (cmd == Read && selective_vc) {
		...
		/*maheshma - case 2*/
		if (strcmp(cp->name, cache_dvc->name) == 0
				|| strcmp(cp->name, cache_ivc->name) == 0) {
			if (prev_blk->sticky_bit == 0) {
				move(*repl_addr, blk, cache_vc);
				move(addr, prev_blk, cache_l1);
				//maheshma - also need to swap the block status so if there is any Dirty block set it is appropriately moved/handled
		...
		/*maheshma - case 3*/
		if (strcmp(cp->name, cache_dl2->name) == 0
				|| strcmp(cp->name, cache_il2->name) == 0) {
			if (prev_blk->sticky_bit == 0) {
				//move alpha to victim cache
				// Need to find the right block in vc first
			...
			if (blk->status & CACHE_BLK_VALID)
			{
				...
				if (blk->status & CACHE_BLK_DIRTY)
				{
					...
					lat += cp->blk_access_fn(Write,
							CACHE_MK_BADDR(cp, blk->tag, i), cp->bsize, blk,
							now + lat, NULL);
				}
			}
		}
